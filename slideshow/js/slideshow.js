...
const createSlideshow = function () {
    ...
    return {
        ...
        createSetSpeedHandler: function () {
            let me = this
            return function () {
                collectNewSpeed = prompt(`Current speed is ${me.getSpeed()/1000} seconds, enter a new speed in seconds.`)
                if (collectNewSpeed) {
                    me.setSpeed(collectNewSpeed*1000)
                    stopSlideShow()
                    me.startSlideShow()
                } 
            }
...
window.addEventListener('load', () => {
    ...
    //SET SPEED
    $('setSpeed').onclick = slideshow.createSetSpeedHandler()
})
